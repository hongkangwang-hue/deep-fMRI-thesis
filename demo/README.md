...
3. TR 时间轴（去 pad）      (256,)        respdict=261 − pad 5 帧(10s) = 256
4. Lanczos 重采样 词→TR    (256, 768)    不等间隔词 → 等间隔 TR 网格（TR=2s）
5. 裁边 [10:-5]           (241, 768)    再去头 10 帧(20s)、去尾 5 帧(10s)
   ↳ 累计裁切              261−5−10−5=241  累计去头 15 帧(30s)、去尾 5 帧(10s)
   ↳ 行数一致性            241 == 241    一致（assemble 内为硬断言）
6. [折内] scaler + PCA     (T, 100)      降到 100 维，仅用训练折拟合
7. FIR 四延迟展开          (241, 400)    维度 ×4 = 100×4
   ↳ FIR 有效 TR           237 / 241     前 4 帧因零填充无效
8. 评分掩码 ∩ >100s        196 / 241     最终参与评分的 TR 数
总结：1528 个词级向量 → 241 个 TR 行 → 400 列设计矩阵 → 196 个 TR 参与打分
```

> **裁切分两步,别把单步和累计搞混**(这里曾出过一处表述错误):
> `story_tr_times` 先去掉 pad 的 5 帧(10 s),`[10:-5]` 再去头 10 帧(**20 s**)、去尾 5 帧(10 s)。
> 因此 **单步**去头是 20 s,**累计**去头才是 15 帧 = 30 s。
> 累计口径 `261−5−10−5=241` 与论文 "BOLD preprocessing removed the first 15 and last 5 TRs"
> 严格对应。脚本现在两行都打印,避免误读。
...
UTS02         +0.004395     [+0.002601, +0.006029]    <0.002          +0.0044       ✓
...
> **p 值分辨率(论文需同步修订的一处)**:双尾 p 的算法是 `p = min(1, 2k/B)`
> ([src/stats/bootstrap.py](../src/stats/bootstrap.py) 的 `bootstrap_two_sided_p`),
> `B=1000` 时 p 只能取 `0, 0.002, 0.004, …` 这些离散值。当没有任何 draw 跨过零时,
> 代码返回的是**精确的 0.0**,能如实声明的只是 `p < 2/B = 0.002`。
> 写 `p<0.001` 会暗示 `B=2000` 才有的分辨率,属于过度声明。
> D3 已按 `p<0.002` 输出;**论文表 4 与 Methods 中的 4 处 `p<0.001` 建议一并改为
> `p<0.002`**。所有 Holm 判定不变(`0.002 < 0.025 < 0.05`),不影响任何结论。
