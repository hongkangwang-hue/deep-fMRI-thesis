...
- **UTS02**：即便用这个口径，仍差 0.000288。最后一步诊断（那 2 个落在 IFG 里的差异体素具体数值）**已完成**，确认属于同一类"符号翻转"现象，不是新的独立问题——诊断链本身已经完整，但数值上 UTS02 在"限定 voxel_mask"口径下依然没有过 0.995 这条线。

**诊断部分到此结束，但方法学决定仍未做出**：诊断已经完整回答了"差异从哪来、是不是实现错误"（不是；是两类已知、已定位、且几乎不落在 ROI 内的数值边界效应），但"能不能把限定 voxel_mask 的口径当作本次硬闸门的有效判据、UTS02 剩余的 0.000288 缺口算不算可接受"是一个**尚未做出的方法学决定**，不是诊断能替代的。

## 下一步（按顺序）

1. **将本文件这部分诊断链整理成正式的方法学记录**（内容已经具备，需要的是把它组织成可以直接放进论文 Methods/Limitations 或者独立呈交导师的文档形式），明确写清楚三件事：官方全体素判据 FAIL 的根本原因、限定 voxel_mask 口径是否可以作为本次验证的有效解释、UTS02 剩余 0.000288 缺口如何处理。
2. 这类偏离预注册硬闸门的决定，建议同步给导师，不宜由用户和 Claude 私下定案。
3. M2 正式结案（无论最终以何种口径、附带何种书面说明）之后，才能进入 M3（新被试端到端竖切复核）。
