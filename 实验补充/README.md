...
这条建议而**新增**的代码，不修改主实验（`src/`、`scripts/`）的任何已有文件（除 §3
提到的一处可选参数）。
...
| 1 | 上下文置换纯函数 + 单测 | 无 | ✅ `src/context_perturb.py` + `tests/` |
| 2 | 硬闸门断言 + 词形重复诊断（§2.4） | 无 | ✅ `scripts/m4s_assert_impl.py` → `results/` |
| 3 | 打乱上下文特征重提取 | GPU | ⏸ 待你确认 |
| 4 | ctx1 条件 Ridge 重拟合 | Ridge | ⏸ 待你确认 |
| 5 | 统计量 `D_m`/`I_MP` + bootstrap CI | 轻量 | ⏸ 待你确认 |
| 6 | 论文 Figure 17 / Table 9 | 无 | ⏸ 待步骤 5 完成 |

Step 0–2 全部本地零算力，已产出真实结果（见 `results/`）。Step 3 起需要服务器计算，
按项目规矩需你确认后再执行。
